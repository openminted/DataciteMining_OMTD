    ...
    def VTiter(self, *args,**formatArgs):
        ...
        if mode == 'spac':
            ...
            while True:
                ...
                column = msgpack.loads(zlib.decompress(input.read(ind[0*4])))
                ...
                if ind[col*4+2] == 0: 
                    myvals[col] = column                  
                    if ("8.725" >= column[0] or "8.725"<=column[len(column)-1]):
                        t = binarySearch(column,"8.725")
                        if t[1]:
                            found = 1
                            index_found = t[0]
				...
				listptr = array('i')
                     ...
                     if found:
                        if len(myvals[col])<256:
                                listptr = array('B')
                                listptr.fromstring(zlib.decompress(input.read(ind[col*4+1])))
                                indices = [i for i, x in enumerate(listptr) if x == index_found]
                                for i in indices:
                                    d2[col].append("8.725")
                        elif len(myvals[col])<65536:
                                listptr = array('H')
