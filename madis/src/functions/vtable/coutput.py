...
#    print zstd.strlen(lala)
#    zstd.ZSTD_decompress(kaka, 100, lala, zstd.strlen(lala)+1 )
#    print zstd.strlen(kaka)
    import bz2
    import msgpack
    serializer = msgpack
    ### Parameter handling ###
    ...
    def spac1(fileObject,lencols,compression,level):
        colnum = len(schema)-1
        cPickle.dump(schema[1:],fileObject)
	    ...
	    #type = '!'+'i'*len(headindex2)
            ...
            if mrows != []:
                ...
                for i, col in enumerate(([x[c] for x in mrows] for c in xrange(colnum))):
                    if blocknum==0:
                        ...
                        if lens>50*1.0*count/100:
                            ...
                            coldict = dict(((x,y) for y,x in enumerate(globaldict[i])))
                            l = output.tell()
                            headindex[i*4+3] = l
                            output.write(compress(serializer.dumps(globaldict[i]),level))
                            valcount[i] += lens
                            headindex[i*4] = output.tell()-l
				...
				l1 = output.tell()
				    ...
				    print 'lala'
                                    output.write(compress(array('i',[coldict[y] for y in col]).tostring(),level))
                                    indexes_size[i] += output.tell() - l1
                            headindex[i*4+1] = output.tell()-l-headindex[i*4]
			    ...
			    so = sorted(set(col))
                            ...
                            diff = setcol - set(globaldict[i])   # upologismos newn diaforetikwn val
			    lens = len(globaldict[i])
                            if lens > 2147483647 or (lens < 65535 and lens + len(diff) > 65535) or (lens<256 and lens + len(diff) >= 256) or len(setcol) == 1 or  (lens>=256 and len(setcol) < 256):               
                                headindex[i*4+2] = 0
                                # sdc again
                                globaldict[i] = so
                                coldict = dict(((x,y) for y,x in enumerate(globaldict[i])))
				...
				lens = len(globaldict[i])
				    ...
				    indexes_size[i] += output.tell() - l1
                                ...
                                dif = sorted(diff)
                                if dif:
                                    globaldict[i] = globaldict[i] + dif # upologismos neou global dict
					...
					indextype = 'i'
                                l = output.tell()
                                headindex[i*4+3] = l
                                output.write(compress(serializer.dumps(dif),level))
                                ...
                                if lens>1:
	                            l1 = output.tell()
                                    output.write(compress(array(indextype,[coldict[y] for y in col]).tostring(),level))
                                    indexes_size[i] += output.tell() - l1
...
#                            fastPickler.dump(s)
			    ...
			    #di = dict(zip(s,[x for x in xrange(len(s))]))
			    #di["min"] = s[0]
			    #di["max"] = s[len(s)-1]
		            #output.write(compress(serializer.dumps(di),level))
                            valcount[i] += len(s)
                            headindex[i*2] = output.tell()-l
		            values_size[i] += headindex[i*2]
                            if lens>1:
				l1 = output.tell()
                                if lens<256:
                                    output.write(compress(array('B',[coldict[y] for y in col]).tostring(),level))
...
#                    fastPickler.dump(col)
    ...
    def calclencols(mode):
            ...
            if mode==SDC or mode==SPAC:
                ...
                rows = []
                try:
                    while bsize<BLOCK_SIZE and count<MAX_RECORDS:
                        row = diter.next()
                        rows.append(row)
                        count += 1
                        if split:
    ...
    if mode == 'spac':
        if 'split' in formatArgs:
            ...
            spacgen = [spac(x,lencols,compression,level) for x in filesList]
